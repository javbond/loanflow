...
class DelegateTests {
    ...
    class CreditCheckDelegateTests {
        ...
        @Test
        @DisplayName("Should execute credit check and set default score")
        void shouldExecuteCreditCheck() {
            UUID appId = UUID.randomUUID();
            ...
            assertThat(app.getRiskCategory()).isEqualTo("MEDIUM");
            verify(execution).setVariable("cibilScore", 750);
            verify(execution).setVariable("riskCategory", "MEDIUM");
            verify(repository, atLeast(2)).save(any());
        }

        @Test
        @DisplayName("Should preserve existing CIBIL score")
        void shouldPreserveExistingScore() {
            UUID appId = UUID.randomUUID();
            LoanApplication app = buildApplication(appId, LoanStatus.DOCUMENT_VERIFICATION);
            app.setCibilScore(800);
            ...
            assertThat(app.getCibilScore()).isEqualTo(800);
            assertThat(app.getRiskCategory()).isEqualTo("LOW");
            verify(execution).setVariable("cibilScore", 800);
